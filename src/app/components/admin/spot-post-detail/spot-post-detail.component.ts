...
@Component({
  selector: 'app-spot-post-detail',
  templateUrl: './spot-post-detail.component.html',
  styleUrls: ['./spot-post-detail.component.css','../../../../../node_modules/admin-lte/bower_components/bootstrap/dist/css/bootstrap.min.css',
...
export class SpotPostDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get('https://ibigo.shadowis.nl/server-api/api/admin/spot-post/'+id).subscribe((data)=>{
      if(data['status']==true){
        this.post = data['post'][0];
        this.likes_users = this.post.liked_users_names.length;
        if(this.post.liked_by_logged_in_user == 1){
          this.likes_users = parseInt(this.likes_users)+1;
        }
      }
    });
  }
